...
function getBookById(books, id) {
  let book = books.map((elem) => elem.id);
  try {
    for (let i of book) {
      if (i === id) {
        return `Книга ${id}`;
...
function decodeURIComponentWrapper(encodedString) {
  try {
    let res = decodeURIComponent(encodedString);
    return res;
  } catch (err) {
...
function findEvenNumber(numbers) {
  console.log(numbers);
  let evenNumber = numbers.find((elem) => elem % 2 === 0);
  try {
    if (!evenNumber)
...
function validateUser(user) {
  try {
    ...
    return console.error(err.message, err.cause);
  }

  // Перевіряємо, чи існує об'єкт користувача,якщо ні викидуємо помилку з повідомленням "Об'єкт не знайдено!" .
  // Перевіряємо, чи існує ім'я користувача,якщо ні викидуємо помилку з повідомленням , а як причину вказуємо об'єкт user.
  ...
  try {
    for (const index of data) {
      if (typeof index !== "number") {
        ...
        error.message = `Елемент з індексом ${index} має бути числом!`;
        throw error;
      }
    }
  } catch (err) {
